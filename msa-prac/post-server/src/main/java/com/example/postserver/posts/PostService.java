...
public class PostService {
    ...
    private SocialPost updateUploaderName(SocialPost post) {
        UploaderInfo uploaderInfo = uploaderService.getUserInfo(post.getUploaderId());
        return new SocialPost(post, uploaderInfo.getUsername());
    }
    public List<SocialPost> getAllPosts() {
        return postRepository.findAll()
            .stream()
            .map(SocialPost::fromPostEntity)
            .map(this::updateUploaderName)
            .toList();
    }

    public List<SocialPost> getAllPostsByUploaderId(int uploaderId) {
        return postRepository.findByUploaderId(uploaderId)
            .stream()
            .map(SocialPost::fromPostEntity)
            .map(this::updateUploaderName).toList();
    }
    ...
    public SocialPost createPost(PostRequest post) {
        ...
        PostActivity message = new PostActivity(savedPost.getUploaderId(), savedPost.getUploadDatetime(),
            String.valueOf(savedPost.getPostId()));
